...
void AIInitDMA(u32 address, u32 length)
{
    ...
    __DSPRegs[DSP_DMA_START_LO] =
        (u16)((__DSPRegs[DSP_DMA_START_LO] & ~0xFFE0) | (address & 0xFFFF));
    __DSPRegs[DSP_DMA_CONTROL_LEN] =
        (u16)((__DSPRegs[DSP_DMA_CONTROL_LEN] & ~0x7FFF) | ((length >> 5) & 0xFFFF));
...
void AIStopDMA(void)
{
    RESET_FLAG(__DSPRegs[DSP_DMA_CONTROL_LEN], DSP_DMA_START_FLAG);
}
...
{
    __AIRegs[AI_INTRPT_TIMING] = trigger;
}
...
void AISetStreamPlayState(u32 playState)
{
    ...
    if (AIGetStreamSampleRate() == 0 && playState == TRUE)
    {
        ...
        __AIRegs[AI_CONTROL] = (__AIRegs[AI_CONTROL] & ~AI_CONTROL_STREAM_SAMPLE_COUNT) |
                               AI_CONTROL_STREAM_SAMPLE_COUNT;
        __AIRegs[AI_CONTROL] =
            (__AIRegs[AI_CONTROL] & ~AI_CONTROL_PLAY_STATE) | AI_CONTROL_PLAY_STATE;
...
void AISetDSPSampleRate(u32 rate)
{
    ...
    if (rate == 0)
    {
        ...
        __AIRegs[AI_CONTROL] = (__AIRegs[AI_CONTROL] & ~AI_CONTROL_STREAM_SAMPLE_COUNT) |
                               AI_CONTROL_STREAM_SAMPLE_COUNT;
        __AIRegs[AI_CONTROL] =
            (__AIRegs[AI_CONTROL] & ~AI_CONTROL_STREAM_SAMPLE_RATE) | (streamSampleRate * 2);
        __AIRegs[AI_CONTROL] = (__AIRegs[AI_CONTROL] & ~AI_CONTROL_PLAY_STATE) | playState;
